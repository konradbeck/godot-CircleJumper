...
func init(_position, level=1):
	var _mode = Settings.rand_weighted([3, level - 1])
	set_mode(_mode)
	position = _position
	var move_chance = clamp(level - 3, 0, 9) / 3.0
	if randf() < move_chance:
		move_range = max(25, 100 * rand_range(0.75, 1.25) * move_chance) * pow(-1, randi() % 2)
		move_speed = max(2.5 - ceil(level / 5) * 0.25, 0.75)
	var small_chance = min(0.9, max(0, (level - 3) / 20.0))
	if randf() < small_chance:
		radius = max(50, radius - level * rand_range(1.75, 2.25))
	$Sprite.material = $Sprite.material.duplicate()
	$SpriteEffect.material = $Sprite.material
	$CollisionShape2D.shape = $CollisionShape2D.shape.duplicate()
	$CollisionShape2D.shape.radius = radius
	var img_size = $Sprite.texture.get_size().x / 2
	$Sprite.scale = Vector2.ONE * radius / img_size # ratio of radius to image size
